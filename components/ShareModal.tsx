                ...
                {soulmates.map(thread => (
                    <li key={thread.id} className="flex items-center justify-between">
                        <div className="flex items-center">
                            <img src={thread.user.avatarUrl} alt={thread.user.name} className="w-12 h-12 rounded-full mr-3"/>
                            <span className="font-semibold text-text-primary">{thread.user.name}</span>
                        </div>
                        <button onClick={() => onShare(thread.user)} className="bg-primary text-white px-4 py-1.5 rounded-full text-sm font-semibold hover:bg-primary-dark transition-colors">
                            Send
                        </button>
                    </li>
                ))}
