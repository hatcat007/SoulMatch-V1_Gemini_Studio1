  ...
  return (
    <div className="min-h-screen w-full bg-primary-light flex flex-col items-center justify-center p-4 overflow-hidden">
        ...
        <main className="relative flex-1 flex items-center justify-center w-full max-w-4xl">
            ...
            >
                {/* Left Phone */}
                <motion.div variants={phoneVariants('left')} animate={animationState} transition={{ duration: 1.5, ease: "easeInOut" }}>
                    <PhoneComponent />
                </motion.div>
                
                {/* Right Phone */}
                <motion.div variants={phoneVariants('right')} animate={animationState} transition={{ duration: 1.5, ease: "easeInOut" }}>
                    <PhoneComponent />
                </motion.div>
                ...
                <div className="absolute">
                    <AnimatePresence>
                        {animationState === 'connecting' && (
                            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} className="flex items-center justify-center">
                                <Wifi className="text-primary" style={{ width: `${phoneWidth * 0.5}px`, height: `${phoneWidth * 0.5}px` }}/>
                                {connectionSpikes.map((_, i) => (
                                    <motion.div key={i} custom={i} variants={spikeVariants} initial="hidden" animate="visible" exit="hidden"
