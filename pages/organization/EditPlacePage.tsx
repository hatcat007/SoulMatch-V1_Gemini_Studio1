...
import type { Place, ImageRecord, Activity, Interest, InterestCategory } from '../../types';
...
const EditPlacePage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchPlaceAndTags = async () => {
            ...
            if (fetchError || !data) { setError(`Kunne ikke hente mødested: ${fetchError?.message}`); setLoading(false); return; }
            
            setFormData({
                name: data.name || '', offer: data.offer || '', address: data.address || '', icon: data.icon || '☕',
                category_id: data.category_id, description: data.description || '', phone: data.phone || '',
                opening_hours: data.opening_hours || '', is_sponsored: data.is_sponsored || false,
                user_count: data.user_count?.toString() || '0'
            });
            setImages(data.images || []);

            const activitiesPromise = supabase.from('activities').select('*').eq('approved', true).order('name');
            const selectedActivitiesPromise = supabase.from('place_activities').select('activity_id').eq('place_id', placeId);
            const interestCategoriesPromise = supabase.from('interest_categories').select('*').order('name');
            const interestsPromise = supabase.from('interests').select('*').eq('approved', true).order('name');
            const selectedInterestsPromise = supabase.from('place_interests').select('interest_id').eq('place_id', placeId);

            const [actRes, selActRes, intCatRes, intRes, selIntRes] = await Promise.all([activitiesPromise, selectedActivitiesPromise, interestCategoriesPromise, interestsPromise, selectedInterestsPromise]);
            
            setAllActivities(actRes.data || []);
            setSelectedActivityIds(selActRes.data?.map(link => link.activity_id) || []);
            setInterestCategories(intCatRes.data || []);
            setAllInterests(intRes.data || []);
            setSelectedInterestIds(selIntRes.data?.map(link => link.interest_id) || []);
    ...
    const handleActivityToggle = (activity: Activity) => {
        setSelectedActivityIds(prev => prev.includes(activity.id) ? prev.filter(id => id !== activity.id) : [...prev, activity.id]);
    };

    const handleInterestToggle = (interest: Interest) => {
        setSelectedInterestIds(prev => prev.includes(interest.id) ? prev.filter(id => id !== interest.id) : [...prev, interest.id]);
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const { error: updateError } = await supabase.from('places').update({
            name: formData.name, offer: formData.is_sponsored ? formData.offer : '', address: formData.address, icon: formData.icon,
            category_id: formData.category_id, description: formData.description, phone: formData.phone, opening_hours: formData.opening_hours,
            is_sponsored: formData.is_sponsored, user_count: parseInt(formData.user_count, 10) || 0,
        ...
        if (images.length > 0) {
            const imageRecords = images.map(({ image_url }) => ({ place_id: Number(placeId), image_url }));
            await supabase.from('place_images').insert(imageRecords);
        }

        await supabase.rpc('add_activities_to_place', { p_place_id: Number(placeId), p_activity_ids: selectedActivityIds });
        await supabase.rpc('add_interests_to_place', { p_place_id: Number(placeId), p_interest_ids: selectedInterestIds });
            ...
            <form onSubmit={handleSubmit} className="space-y-4 max-w-2xl mx-auto bg-white dark:bg-dark-surface p-6 rounded-lg shadow-sm">
                 ...
                 <div className="space-y-2">
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        <div className="grid grid-cols-5 sm:grid-cols-9 gap-2 flex-1">{emojiOptions.map(e => (<button key={e} type="button" onClick={() => handleInputChange({ target: { name: 'icon', value: e } } as any)} className={`text-2xl p-2 rounded-lg transition-transform duration-200 ${formData.icon === e ? 'bg-primary-light dark:bg-primary/20 scale-110' : 'hover:bg-gray-100 dark:hover:bg-dark-surface-light'}`}>{e}</button>))}</div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 ...
                 <div>
                    ...
                    <div className="grid grid-cols-3 md:grid-cols-4 gap-2">
                        {images.map(image => (<SmartImage key={image.id} src={image.image_url} alt="Mødested billede" className="w-full h-full object-cover rounded-lg" onRemove={() => removeImage(image.id)} />))}
                ...
                <CategorySelector value={formData.category_id} onChange={(id) => handleInputChange({ target: { name: 'category_id', value: id } } as any)} type="place" />
                
                <TagSelector title="Vælg aktiviteter" categories={[]} allTags={allActivitiesForSelector} selectedTags={selectedActivitiesForSelector} onToggleTag={(tag) => { const act = allActivities.find(a => a.id === tag.id); if (act) handleActivityToggle(act); }} containerHeight="h-auto" />
                ...
                <div className="bg-gray-50 dark:bg-dark-surface-light p-4 rounded-lg">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div><label htmlFor="phone" className="block text-sm font-medium mb-1">Telefon</label><input type="tel" id="phone" name="phone" value={formData.phone} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 dark:bg-dark-surface-light border rounded-lg" /></div>
                    <div><label htmlFor="opening_hours" className="block text-sm font-medium mb-1">Åbningstider</label><input type="text" id="opening_hours" name="opening_hours" value={formData.opening_hours} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 dark:bg-dark-surface-light border rounded-lg" /></div>
                 </div>
                 ...
                 <div>
                    <label htmlFor="user_count" className="block text-sm font-medium mb-1">Gennemsnitligt antal gæster</label>
                    <input type="number" id="user_count" name="user_count" value={formData.user_count} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 dark:bg-dark-surface-light border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary" />
                </div>
                
                <div className="pt-2">
                     <button type="submit" disabled={isSubmitting} className="w-full bg-primary text-white font-bold py-4 px-4 rounded-full text-lg hover:bg-primary-dark transition shadow-lg disabled:opacity-50">
