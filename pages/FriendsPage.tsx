...
const FriendsPage: React.FC = () => {
    ...
    const handleAcceptRequest = async (friendshipId: number, friendId: number) => {
        ...
        const { error: updateError } = await supabase.from('friends').update({ status: 'accepted' }).eq('id', friendshipId);
        if (updateError) { console.error('Error accepting request:', updateError); setRequests(originalRequests); return; }
        const { data: threadId, error: rpcError } = await supabase.rpc('get_or_create_chat_thread', { p_friend_id: friendId });
